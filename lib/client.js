...
const { routeWritingSkills } = require('./writing-skill-router.cjs')
...
function h(type, props, ...children) { return React.createElement(type, props, ...children) }
function renderMessage(message) { if (message.role !== 'assistant') { const route = message.requestMetadata?.skillRoute; return [message.content, route && h('small', { className: 'nv-receipt', key: 'skill' }, `方法：${route.labels?.join(' + ') || route.skills?.join(' + ')} · ${route.reason}`), message.contextSummary && h('small', { className: 'nv-receipt', key: 'receipt' }, `已发送：${message.contextSummary}`)] } const parts = String(message.content || '').split(/(```[\s\S]*?```)/g); return h('div', { className: 'nv-markdown' }, parts.filter(Boolean).map((part, index) => part.startsWith('```') ? h('pre', { key: index }, part.replace(/^```[^\n]*\n?|```$/g, '')) : h('p', { key: index }, part.replace(/\*\*/g, '')))) }
...
function AssistantPanel({ connectionState, mode, onModeChange, runtime, adapter, manifest, onManifest, document, content, revision, selection, proposal, onProposal, onApply, onReject, onUndo, changeSet, saveState, activeChangeId, onActiveChange }) {
  ...
  const skillRoute = routeWritingSkills({ mode, input, selection: selectionText, activeProposal: proposal, changeSet, includeCurrent })
  ...
  return h('aside', { className: 'nv-assistant', 'aria-label': 'Narraiva 助手' },
    ...
    h('div', { className: 'nv-skill-route', 'aria-label': '本轮写作方法' },
      h('strong', null, `方法 · ${skillRoute.labels.join(' + ')}`),
      h('span', null, skillRoute.reason),
      h('span', null, `上下文：${skillRoute.contextScope} · 输出：${skillRoute.outputContract === 'proposal' ? '可审阅 Proposal' : '只读回复'}`),
    ),
...
function NarraivaStatusBar({ connectionState }) {
  return h('footer', { className: 'nv-statusbar' },
    h('span', null, 'Phase 6A · Writing Skill Kernel'),
